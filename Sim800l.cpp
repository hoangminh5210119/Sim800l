...
void Sim800l::begin(Stream &serial) {
  _mySerial = &serial;
}
...
String Sim800l::_readSerial() {
  _timeout = 0;
  while (!_mySerial->available() && _timeout < 12000) {
    delay(13);
    _timeout++;
  }
  if (_mySerial->available()) {
    return _mySerial->readString();
  }
  return "";
}
...
void Sim800l::callNumber(char *number) {
  _mySerial->print(F("ATD"));
  _mySerial->print(number);
  _mySerial->print(F(";\r\n"));
}
...
uint8_t Sim800l::getCallStatus() {
  ...
  */
  _mySerial->print(F("AT+CPAS\r\n"));
  _buffer = _readSerial();
  return _buffer
      .substring(_buffer.indexOf("+CPAS: ") + 7, _buffer.indexOf("+CPAS: ") + 9)
      .toInt();
}

bool Sim800l::hangoffCall() {
  _mySerial->print(F("ATH\r\n"));
  _buffer = _readSerial();
  if ((_buffer.indexOf("OK")) != -1)
    return true;
  else
...
bool Sim800l::sendSms(char *number, char *text) {
  ...
  _mySerial->print(F("AT+CMGF=1\r")); // set sms to text mode
  _buffer = _readSerial();
  _mySerial->print(F("AT+CMGS=\"")); // command to send sms
  _mySerial->print(number);
  _mySerial->print(F("\"\r"));
  _buffer = _readSerial();
  _mySerial->print(text);
  _mySerial->print("\r");
  // change delay 100 to readserial
  _buffer = _readSerial();
  _mySerial->print((char)26);
  _buffer = _readSerial();
...
String Sim800l::readSms(uint8_t index) {
  _mySerial->print(F("AT+CMGF=1\r"));
  if ((_readSerial().indexOf("ER")) == -1) {
    _mySerial->print(F("AT+CMGR="));
    _mySerial->print(index);
    _mySerial->print("\r");
    _buffer = _readSerial();
...
String Sim800l::dateNet() {
  ...
  if (_buffer.indexOf("OK") != -1) {
    return _buffer.substring(_buffer.indexOf(":") + 2,
                             (_buffer.indexOf("OK") - 4));
  } else
    return "0";
}
...
bool Sim800l::updateRtc(int utc) {
  ...
  _mySerial->print("at+cclk=\"" + dt.substring(2, 4) + "/" + dt.substring(5, 7) + "/" +
            tmp_day + "," + tmp_hour + ":" + tm.substring(3, 5) + ":" +
            tm.substring(6, 8) + "-03\"\r\n");
  if ((_readSerial().indexOf("ER")) != -1) {
    return false;
  } else
